"""Download ERA5 monthly-mean SST over the tropical Pacific for 1980 on a 2x2 grid.

Same request as download_era5_sst_pacific_1980.py but regridded to 2 x 2 degrees
via the CDS 'grid' key.

Region : tropical Pacific  (30N-30S, 120E-80W)  ->  area = [N, W, S, E]
Variable: sea_surface_temperature (K)
Output : NetCDF + long-format CSV + metadata JSON under data/ERA5_sst_pacific_2x2/
"""
...
# --- Configuration ---
DATA_DIR = os.path.join(os.path.dirname(__file__), "..", "data", "ERA5_sst_pacific_2x2")
...
NC_PATH = os.path.join(DATA_DIR, f"era5_sst_pacific_2x2_{YEAR}.nc")
CSV_PATH = os.path.join(DATA_DIR, f"era5_sst_pacific_2x2_{YEAR}.csv")
META_PATH = os.path.join(DATA_DIR, f"era5_sst_pacific_2x2_{YEAR}_metadata.json")


def download(retries=3, wait=60):
    """Download the ERA5 SST NetCDF on a 2x2 grid; reuse if already present."""
